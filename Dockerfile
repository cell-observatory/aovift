...
# docker build . -t ghcr.io/cell-observatory/aovift:main_TF_CUDA_12_3 --build-arg BRANCH_NAME=$(git branch --show-current) --target TF_CUDA_12_3 --progress=plain
#
...
# install github self-hosted runner: https://github.com/cell-observatory/aovift/settings/actions/runners/new?arch=x64&os=linux
...
# docker run --rm -it --gpus all --ipc=host --cap-add=SYS_ADMIN --privileged=true --security-opt seccomp=unconfined --ulimit memlock=-1 --ulimit stack=67108864  -u 1000 -v ${HOME}/.ssh:/sshkey -v ${PWD}:/app/aovift  ghcr.io/cell-observatory/aovift:main_TF_CUDA_12_3 /bin/bash
...
# docker run --rm kaczmarj/apptainer pull main_TF_CUDA_12_3.sif docker://ghcr.io/cell-observatory/aovift:main_TF_CUDA_12_3
...
ENV cloneit="git clone -b ${BRANCH_NAME} --recurse-submodules https://github.com/cell-observatory/aovift.git"
RUN echo 'alias repo="cd /app/aovift/src"' >> ~/.bashrc
...
# ADD "https://api.github.com/repos/cell-observatory/aovift/commits?sha=${BRANCH_NAME}&per_page=1" dummy_location_one

# git clone "requirements.txt" into a junk directory, then pip install. --filter=blob:none will only download the files in HEAD
WORKDIR /docker_install
ADD https://raw.githubusercontent.com/cell-observatory/aovift/${BRANCH_NAME}/requirements.txt requirements.txt
RUN echo "branch=${BRANCH_NAME}" && git clone -n -b ${BRANCH_NAME} --depth 1 --filter=blob:none https://github.com/cell-observatory/aovift.git
WORKDIR /docker_install/aovift
...
# Our repo location will be /app/aovift
